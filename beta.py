...
lbl_tablaPendientes = Label(inicio, text="Pacientes", font=("Verdana",17), bg="#CEE1F2")
lbl_tablaPendientes.place(x=650,y=10)
...
lbl_tablaDespachados = Label(inicio, text="Alertas", font=("Verdana",17), bg="#CEE1F2")
lbl_tablaDespachados.place(x=650,y=300)
...
alertas_table.heading("#0",text="RUT", anchor=W)
...
def guardarPaciente(Rut,Nombre,Apellido,Fono,Direccion,Patologia,Observacion):
    pte=Paciente(Rut,Nombre,Apellido,Fono)
 #   fch=Ficha_Medica(pte,)
    pte.setRut(Rut.get())
    pte.setNombre(Nombre.get())
    pte.setApellido(Apellido.get())
    pte.setFono(Fono.get())
